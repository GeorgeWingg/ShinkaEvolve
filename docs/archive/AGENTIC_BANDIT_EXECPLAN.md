...
### Phase 3: Testing
- [x] Create unit test for `BackendBandit` configuration and sampling.
  - Evidence: `uv run pytest -q tests/test_backend_bandit.py tests/test_agentic_bandit_extensions.py` (23 passed, 2025-12-11).
...
## Verification
- [x] Run short bandit evolutions in agentic mode.
  - Multi-backend run (Gemini + Claude only): `results/shinka_circle_packing/2025.12.11223445_example`
  - Auth-filtering run (Jules unauth + Claude): `results/shinka_circle_packing/2025.12.11223621_example`
- [x] Verified different backends are used in multi-backend run (Gemini and Claude selected across generations).
  - Evidence: `launch_hydra.log` lines showing alternating selections and changing posteriors.
